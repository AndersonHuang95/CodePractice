...
def find(subsets, cell):
    if not isinstance(cell, tuple) and len(cell) != 2:
        raise ValueError

    row, col = cell[0], cell[1]
    if subsets[row][col].parent != cell:
        subsets[row][col].parent = find(subsets, subsets[row][col].parent)
    return subsets[row][col].parent

def union(subsets, first, second):
    first_root = find(subsets, first)
    second_root = find(subsets, second)

    first_subset = subsets[first_root[0]][first_root[1]]
    second_subset = subsets[second_root[0]][second_root[1]]

    if first_subset.rank > second_subset.rank:
        second_subset.parent = first_root
    elif first_subset.rank < second_subset.rank:
        first_subset.parent = second_root
    else: 
        second_subset.parent = first_root
        first_subset.rank += 1
...
class Solution:
    def numIslands2(self, m, n, positions):
        ...
        subsets = [[Subset((-1, -1), 0) for _ in range(n)] for _ in range(m)]
        islands = 0
        ans = []
        for row, col in positions:
            subsets[row][col].parent = (row, col)
            subsets[row][col].rank = 1
            islands += 1

            neighbor_islands = set()
            for direction in dirs:
                nrow, ncol = row + direction[0], col + direction[1]
                if nrow >= 0 and nrow < m and ncol >= 0 and ncol < n and subsets[nrow][ncol].rank > 0:
                    neighbor_islands.add(find(subsets, (nrow, ncol)))

            for direction in dirs:
                nrow, ncol = row + direction[0], col + direction[1]
                if nrow >= 0 and nrow < m and ncol >= 0 and ncol < n and subsets[nrow][ncol].rank > 0:
                    union(subsets, (row, col), (nrow, ncol))
            islands -= len(neighbor_islands)
            ans.append(islands)
        return ans

    def showBoard(self, m, n, positions):
